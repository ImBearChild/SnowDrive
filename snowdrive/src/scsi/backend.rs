...
//! seam from [`crate::common`].
//!
//! The [`BlockStorage`] trait, [`BlockStorageError`], and [`RamBackend`]
//! live in `crate::common::block_storage` (always available, no feature
//! gate). This module adds the std file backend and the aggregating
//! [`BlockBackend`] enum (`Ram` | `File`) so callers can drive a
//! `crate::block::BlockDevice` through a single concrete type.
//! `snowdrive::scsi::backend::{BlockStorage, BlockStorageError, RamBackend,
//! FileBackend, BlockBackend}` is the single import point for the
//! `scsi` feature.

...
//! [`crate::spc`]; this module parses the SBC-specific opcodes and wraps the
//! SPC fall-through as [`SbcCommand::Spc`] so a device's `do_cmd` is a single
...
use crate::spc::{parse_spc, SpcCommand};

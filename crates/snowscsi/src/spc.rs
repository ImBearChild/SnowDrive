...
use crate::device::{CommandOutcome, DeviceType};
use crate::scsi::{asc, cdb_opcode, op, Sense, SenseKey};
...
/// the spc.rs unit-test device). `0x3F` returns every supported page.
pub(crate) fn block_mode_page(page: u8) -> Option<&'static [u8]> {
    match page {
